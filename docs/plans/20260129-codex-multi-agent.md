# 計画: Codex向けマルチエージェント基盤（Windowsネイティブ / MAUI）

## Goal
WindowsのPowerShellから扱いやすく、Codex CLI を複数起動・統率できる基盤を `MonochromeMemory.CodexMultiAgent/` に新規作成する。役割は `config/roles.yaml` で定義し、MAUI Blazor UI で編集・運用できる。
...
- [ ] MAUI Blazor アプリ基盤の作成（UI + API + データアクセス）
